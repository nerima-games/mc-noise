...
正しい修正は勾配集合を大きくすること（3D カーネルが既に使っている 12 方向）。
ただしこれは**凍結された seed→値 の写像の変更**なので、意図的な MAJOR bump まで保留する
（`versioning.md`）。

それまでの回避策: mc-worldgen はブロック中心ではなくブロック原点、
あるいは無理数的なオフセットを足した座標でサンプルすること。
